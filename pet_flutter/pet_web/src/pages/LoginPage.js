...
import { Card, Typography, Button, Row, Col, Space, message, Badge } from 'antd';
...
const LogoPaw = styled(motion.div)`
  position: absolute;
...
const LoginPage = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setPawsCount(prevCount => (prevCount < 12 ? prevCount + 1 : 0));
    }, 3000);
  ...
  // Generate random positions for paw prints
  const getPawPosition = (index) => {
    const positions = [
      { left: '10%', top: '20%' },
      { left: '25%', top: '15%' },
      { left: '40%', top: '25%' },
    ...
    <PageContainer>
      ...
      >
        {showPromo && (
          ...
          >
            <Badge.Ribbon 
              text={
                <Space>
                  <GiftOutlined /> Ưu đãi mới <StarFilled />
                </Space>
              } 
              color="#ff4d4f"
              placement="end"
        ...
        <StyledCard>
          ...
          <StyledSubtitle>
            Vui lòng đăng nhập để chăm sóc thú cưng của bạn
          </StyledSubtitle>
